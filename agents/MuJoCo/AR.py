...
class Args:
    ...
    wandb_project_name: str = "ppo_gym_cleanrl"
...
class Agent(nn.Module):
    def __init__(
        ...
        self.state_embed = nn.Sequential(nn.Linear(state_size, hidden_size), nn.ReLU())
        self.max_action_dim = max(action_dims)
        self.action_embed = nn.Embedding(self.max_action_dim + 1, action_embedding_dim)
        self.start_token_idx = self.max_action_dim
        self.lstm = nn.LSTM(
            hidden_size + action_embedding_dim, lstm_hidden_size, batch_first=True, num_layers=lstm_hidden_layers
        )
        self.output_heads = nn.ModuleList(
            [nn.Linear(lstm_hidden_size, d) for d in action_dims]
        )

    def get_value(self, x: torch.Tensor):
        return self.critic(x)

    def get_action_and_value(self, x: torch.Tensor, multi_action=None):
        batch_size, device = x.shape[0], x.device
        value = self.critic(x)
        state_emb = self.state_embed(x)
        ...
        prev_a_emb = self.action_embed(
            torch.full((batch_size, 1), self.start_token_idx, device=device)
        ).squeeze(1)

        total_logp = torch.zeros(batch_size, device=device)
        ...
        provided = (
            list(torch.split(multi_action.long(), 1, dim=1))
            if multi_action is not None
            else None
        )

        for i, head in enumerate(self.output_heads):
            prefix_logps.append(total_logp)

            lstm_in = torch.cat([state_emb, prev_a_emb], dim=1).unsqueeze(1)
            ...
            if multi_action is None:
                a_i = dist.sample()
                sampled_actions.append(a_i.unsqueeze(1))
            else:
                a_i = provided[i].squeeze(1)
            ...
            total_logp += logp_i
            total_ent += ent_i
            per_dim_ents.append(ent_i.detach())
            prev_a_emb = self.action_embed(a_i)

        actions_out = (
            torch.cat(sampled_actions, dim=1) if multi_action is None else multi_action
        )
        prefix_lp_tensor = torch.stack(prefix_logps, dim=1)
        per_dim_ent_tensor = torch.stack(per_dim_ents, dim=1)
    ...
    for update in range(num_updates):
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue, prefix_lp, per_dim_ent = agent.get_action_and_value(
                ...
                ent_correction = (per_dim_ent * prefix_lp).sum(dim=1).mean()
                total_entropy_term = entropy_loss + ent_correction

                loss = pg_loss - args.ent_coef * total_entropy_term + v_loss * args.vf_coef
...
if __name__ == "__main__":
    ...
    run_name = f"AR-{size}__{args.seed}__{int(time.time())}"
    ...
    agent = Agent(
        ...
        lstm_hidden_layers=args.lstm_hidden_layers
    ).to(device)
